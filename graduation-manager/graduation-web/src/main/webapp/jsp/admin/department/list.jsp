...
<body>
	<table id="dg" title="部门管理" class="easyui-datagrid"
		...
		<thead>
			<tr>
				<th field="name" width="40"><font face="Calibri">部门名称</font></th>
				<th field="description" width="250" align="center"><font face="Calibri">部门描述</font></th>
				<th field="temp1" width="30"><font face="Calibri">备注</font></th>
		</thead>
	</table>
	<div id="toolbar">
		<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true"
			onclick="newUser()">创建部门</a>&nbsp;&nbsp;<a href="#"
			class="easyui-linkbutton" iconCls="icon-edit" plain="true"
			onclick="editUser()">编辑部门</a> &nbsp;&nbsp;<a href="#"
			class="easyui-linkbutton" iconCls="icon-remove" plain="true"
			onclick="destroyUser()">删除部门</a>
...
<script>
	...
	function newUser() {
		...
		$('#fm').form('clear');
		url = '/admin/insertDepartment.htm';
	}
	function saveUser() {
		$('#fm').form('submit', {
			url : url,
	...
	function editUser() {
		...
		if (row) {
			...
			$('#fm').form('load', row);
			url = '/admin/updateDepartment.htm?id=' + row.departmentid;
		}
	}
		...
		if (row) {
			$.messager.confirm('Confirm',
					'Are you sure you want to destroy this user?', function(r) {
						if (r) {
							$.post('/admin/deleteDepartment.htm', {
								id : row.departmentid
							}, function(result) {
								   if(result=="success"){
								       $('#dg').datagrid('reload');
