			...
			<table class="table table-bordered" style="border:1px solid #ccc;">
				<tr>
					<td class="active" colspan="8"><font
						style="color:#000;font-weight: 500;font-size:20px;">考勤信息</font>
					</td>
				</tr>
				<tr>
					<td>编号</td>
				...
				<c:forEach items="${list }" var="salary" varStatus="index">
					<tr>
						<td>${index.index+1 }</td>
						<td>${salary.ontime==1?'是':'否'}</td>
						<td>${salary.aleave}</td>
						<td>${salary.late }</td>
						<td>${salary.missing}</td>
						<td>${salary.goout }</td>
						<td>${salary.plus}</td>
						<td><fmt:formatDate value="${salary.checktime }" pattern="yyyy-MM-dd"/> </td>
					</tr>
				</c:forEach>
				<tr>
